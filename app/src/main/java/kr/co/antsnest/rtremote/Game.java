...
import kr.co.antsnest.rtremote.antsnest.RtSessionRecoveryPolicy;
import kr.co.antsnest.rtremote.antsnest.RtDiagnostics;
...
import kr.co.antsnest.rtremote.binding.input.KeyboardTranslator;
import kr.co.antsnest.rtremote.binding.input.capture.InputCaptureManager;
import kr.co.antsnest.rtremote.binding.input.capture.InputCaptureProvider;
import kr.co.antsnest.rtremote.binding.input.touch.AbsoluteTouchContext;
import kr.co.antsnest.rtremote.binding.input.touch.RelativeTouchContext;
import kr.co.antsnest.rtremote.binding.input.driver.UsbDriverService;
import kr.co.antsnest.rtremote.binding.input.evdev.EvdevListener;
import kr.co.antsnest.rtremote.binding.input.touch.TouchContext;
import kr.co.antsnest.rtremote.binding.input.virtual_controller.VirtualController;
import kr.co.antsnest.rtremote.binding.video.CrashListener;
import kr.co.antsnest.rtremote.binding.video.MediaCodecDecoderRenderer;
import kr.co.antsnest.rtremote.binding.video.MediaCodecHelper;
import kr.co.antsnest.rtremote.binding.video.PerfOverlayListener;
import kr.co.antsnest.rtremote.nvstream.NvConnection;
import kr.co.antsnest.rtremote.nvstream.NvConnectionListener;
import kr.co.antsnest.rtremote.nvstream.StreamConfiguration;
...
public class Game extends Activity implements SurfaceHolder.Callback,
    ...
    private static final String EXTRA_RT_SESSION_RETRY =
            "kr.co.antsnest.rtremote.antsnest.extra.RT_SESSION_RETRY";
    ...
    protected void onDestroy() {
        ...
        if (rtSessionRetryPending && rtSessionRelaunchArmed &&
            ...
            kr.co.antsnest.rtremote.antsnest.RtTunnelManager.close();
        }
    }
    ...
    private synchronized boolean scheduleRtSessionReconnect(final String reason) {
        ...
        RtDiagnostics.record("recovery_evaluate", "reason=" + reason +
                ...
                (kr.co.antsnest.rtremote.antsnest.RtTunnelManager.isActive() ? "connected" : "not-connected") +
                " canReconnect=" + kr.co.antsnest.rtremote.antsnest.RtTunnelManager.canReconnect());
        if (!RtSessionRecoveryPolicy.shouldRetry(
                userInitiatedStop,
                rtSessionRetryPending,
                retryCount,
                connected,
                kr.co.antsnest.rtremote.antsnest.RtTunnelManager.canReconnect())) {
            return false;
        }
        ...
        runOnUiThread(() -> {
            ...
            new Handler(getMainLooper()).postDelayed(() -> {
                ...
                new Thread(() -> {
                    boolean tunnelReady = kr.co.antsnest.rtremote.antsnest.RtTunnelManager.isActive();
                    if (tunnelReady) {
                    ...
                    for (int attempt = 1; !tunnelReady && attempt <= 3 && !userInitiatedStop; attempt++) {
                        ...
                        if (kr.co.antsnest.rtremote.antsnest.RtTunnelManager.reconnect()) {
                    ...
                    runOnUiThread(() -> {
                        ...
                        if (!reconnectSucceeded) {
                            ...
                            Toast.makeText(Game.this,
                                    "RT 터널 재연결에 실패했습니다: " +
                                            kr.co.antsnest.rtremote.antsnest.RtTunnelManager.lastError(),
